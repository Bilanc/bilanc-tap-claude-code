...
class BadCredentialsException(ClaudeCodeException):
    pass
...
def authed_get_all_pages(source, url, headers={}, start_date=datetime.today().strftime('%Y-%m-%d')):
    while True:
        r = authed_get(source, f"{url}?starting_at={start_date}", headers)
        yield r
        if "next" in r.links:
    ...
    with metrics.record_counter(
        ...
        while current_date <= end_date:
            for response in authed_get_all_pages(
                "daily_usage",
                f"{BASE_URL}/organizations/usage_report/claude_code",
...
def do_sync(config, state, catalog):
    ...
    anthropic_api_version = config["anthropic_api_version"] if "anthropic_api_version" in config else "2023-06-01"
...
def main():
    ...
    if not args.config.get("api_key"):
        if os.getenv("CLAUDE_CODE_ADMIN_API_KEY") is not None:
            args.config["api_key"] = os.getenv("CLAUDE_CODE_ADMIN_API_KEY")
        else:
